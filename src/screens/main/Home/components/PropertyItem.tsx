...
import {
  emitFavoriteChanged,
  subscribeFavoriteChanged,
} from '../../../../util/favoriteEvents';
...
export const PropertyItem = ({ dt, onFavoriteChanged }: Props) => {
  ...
  useEffect(() => {
    return subscribeFavoriteChanged((propertyId, favorite) => {
      if (propertyId === dt.item.id) {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      ...
      emitFavoriteChanged(dt.item.id, result.isFavorite);
  ...
  return (
    <View style={styles_.propertContainer}>
      ...
      <View style={styles_.ratingContainer} pointerEvents='none'>
